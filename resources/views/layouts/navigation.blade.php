                ...
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('home') }}" class="group">
                        <div class="flex items-center space-x-2">
                            <div class="w-10 h-10 bg-indigo-600 rounded-lg flex items-center justify-center shadow-[0_0_15px_rgba(79,70,229,0.5)] group-hover:scale-110 transition-transform duration-300">
                        ...
                        <x-dropdown align="right" width="48">
                            <x-slot name="trigger">
                                <button class="inline-flex items-center px-4 py-2 bg-white/5 border border-white/10 text-sm leading-4 font-medium rounded-full text-white hover:bg-white/10 focus:outline-none transition ease-in-out duration-150 backdrop-blur-sm">
                                    <img class="h-6 w-6 rounded-full mr-2" src="https://ui-avatars.com/api/?name={{ urlencode(Auth::user()->name) }}&color=7F9CF5&background=EBF4FF" alt="{{ Auth::user()->name }}" />
                            ...
                            <x-slot name="content">
                                <x-dropdown-link :href="route('profile.edit')" class="hover:bg-indigo-600/20">
                                    {{ __('Profile') }}
                                </x-dropdown-link>
                ...
                @else
                    <div class="flex space-x-4 items-center">
                        ...
                        @if (Route::has('register'))
                            <a href="{{ route('register') }}" class="inline-flex items-center px-5 py-2.5 bg-indigo-600 text-white text-sm font-bold rounded-full hover:bg-indigo-700 shadow-lg shadow-indigo-600/20 transition-all transform hover:-translate-y-0.5">
                                Join Now
                            </a>
                        @endif
                    </div>
        ...
        <div class="pt-4 pb-1 border-t border-white/5">
            @auth
                <div class="px-4 flex items-center">
                    <img class="h-10 w-10 rounded-full mr-3 border-2 border-indigo-500/50" src="https://ui-avatars.com/api/?name={{ urlencode(Auth::user()->name) }}&color=7F9CF5&background=EBF4FF" alt="{{ Auth::user()->name }}" />
                    <div>
            ...
            @else
                <div class="mt-3 space-y-1 p-4">
                    ...
                    <a href="{{ route('register') }}" class="block w-full text-center py-3 bg-indigo-600 text-white font-bold rounded-xl shadow-lg">Join Now</a>
                </div>
            @endauth
        </div>
    </div>
