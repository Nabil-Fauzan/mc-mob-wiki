    ...
    <div class="py-12 px-4 sm:px-6 lg:px-8 relative">
        ...
        <div class="max-w-4xl mx-auto relative z-10">
            ...
            <div class="glass-card rounded-[2.5rem] border border-white/10 p-8 shadow-[0_0_50px_rgba(0,0,0,0.5)] bg-gray-900/40 backdrop-blur-xl">
                <form action="{{ route('admin.biomes.update', $biome) }}" method="POST" enctype="multipart/form-data" class="space-y-8" "
                    x-data="imagePreview('{{ $biome->image ? asset('storage/' . $biome->image) : '' }}')">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <select name="dimension_id" id="dimension_id" required 
                                        ...
                                        <option value="" disabled class="bg-gray-900">Select Dimension Boundary</option>
                                        @foreach($dimensions as $dimension)
                                            <option value="{{ $dimension->id }}" {{ (old('dimension_id', $biome->dimension_id) == $dimension->id) ? 'selected' : '' }} class="bg-gray-900">{{ $dimension->name }}</option>
                                        @endforeach
                                    </select>
                        ...
                        <div>
                            ...
                            <div class="relative group h-[200px] rounded-2xl overflow-hidden border-2 border-dashed border-white/20 bg-black/40 hover:bg-black/60 hover:border-indigo-500/50 transition-all cursor-pointer flex flex-col items-center justify-center" @click="$refs.fileInput.click()">
                                
                                <template x-if="imageUrl">
                                    <img :src="imageUrl" class="absolute inset-0 w-full h-full object-cover z-10" alt="Preview">
                                </template>
                                
                                <div class="z-20 text-center p-4" :class="{'opacity-0 group-hover:opacity-100 transition-opacity bg-black/60 rounded-xl backdrop-blur-sm': imageUrl}">
                                    ...
                                    <span class="text-sm font-bold text-gray-300 block" x-text="imageUrl ? 'Change Image' : 'Select Image File'"></span>
                                </div>
                                
                                <div class="absolute inset-x-0 bottom-0 h-1/2 bg-gradient-to-t from-black/50 to-transparent z-10 pointer-events-none" x-show="imageUrl"></div>
                            </div>
                            
                            <input type="file" name="image" id="image" accept="image/*" class="hidden" x-ref="fileInput" @change="fileChosen">
                    ...
                    <div class="pt-4">
                        ...
                        <textarea name="description" id="description" rows="5" required 
                            ...
                            >{{ old('description', $biome->description) }}</textarea>
                        @error('description') <p class="text-red-400 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>

                    <!-- Action Banner -->
                    <div class="pt-6 border-t border-white/10 flex justify-end items-center gap-4">
                        <a href="{{ route('biomes.show', $biome) }}" class="text-gray-400 hover:text-white font-bold text-sm transition-colors">Cancel Modification</a>
                        ...
                        <button type="submit" class="relative group cursor-pointer">
                            ...
                            <div class="relative px-8 py-4 bg-gray-900 border border-white/10 rounded-xl leading-none flex items-center">
                                <span class="pl-2 flex-1 font-black tracking-widest uppercase text-white group-hover:text-red-400 transition-colors">
                                    Commit Overwrite
                                </span>
