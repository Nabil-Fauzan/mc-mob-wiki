<x-app-layout>
    <div class="py-12 px-4 sm:px-6 lg:px-8 relative">
        ...
        <div class="max-w-4xl mx-auto relative z-10">
            <!-- Header -->
            <div class="mb-8">
                <a href="{{ route('biomes.index') }}" class="inline-flex items-center text-brand-400 hover:text-brand-300 font-bold tracking-widest uppercase text-[10px] mb-6 transition-colors">
            ...
            <div class="glass-card rounded-[2.5rem] border border-white/10 p-8 shadow-[0_0_50px_rgba(0,0,0,0.5)] bg-gray-900/40 backdrop-blur-xl">
                <form action="{{ route('admin.biomes.store') }}" method="POST" enctype="multipart/form-data" 
                    class="space-y-8" 
                    x-data="biomeForm({{ $selectedParent ? $selectedParent->id : 'null' }}, {{ $presetImages->toJson() }}, {{ $uploadedImages->toJson() }})">
                    @csrf
                    ...
                    <div class="pt-4">
                        ...
                        <textarea name="description" id="description" rows="5" required 
                            class="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-gray-600 focus:ring-2 focus:ring-brand-500 focus:border-transparent transition-all overflow-hidden resize-none"
    ...
    <script>
        function biomeForm(initialParentId, presets, uploads) {
            return {
